...
> ✅ — сделано (блок «стабилизация», 2026-09-22).
...
|---|-----|-----|----------|-------------|
| 2.1 | P1 | M | **Vosk как wake-word движок по умолчанию** (`DEFAULT_WAKE_WORD_ENGINE = Vosk`) при том, что код сам пишет «highly not recommended». В режиме `VoiceActive` кадры дублируются в два Vosk-распознавателя (`stt::recognize` + `recognize_wake_word`) — двойная нагрузка на CPU. | Сделать Rustpotter дефолтом (после фикса 1.3); Vosk-wake оставить как fallback. Проверить качество `.rpw` на реальных голосах. |
