...
| ~~2.5~~ ⏹ | P2 | S | **Закрыт без изменений.** Вызывается раз на распознанную фразу (единицы раз в минуту), не на кадр: RwLock + clone короткой строки на этом фоне не измеримы. В `listen_for_command` язык всё же вынесен из цикла. | Кэшировать на время сессии; `get_language` → `Arc<str>` или `Copy`-enum. |
