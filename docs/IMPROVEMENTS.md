...
| ~~1.6~~ ✅ | P1 | S | Изменённые в GUI настройки применяются к работающему ассистенту без перезапуска (см. 4.1). | `crates/jarvis-app/src/main.rs:131` (`ReloadCommands` — TODO) |
...
| ~~3.7~~ ⛔ | P2 | S | **Отклонено — опасно.** Попытка заменить копирование симлинками обнулила 148 файлов в `resources/`: `tauri-build` копирует всё из `bundle.resources` в `target/`, а копирование файла в самого себя (через симлинк обратно в исходники) его обрезает. Восстановлено из git, в `post_build.py` добавлена защита: запись в путь, ведущий обратно в исходники, пропускается. На APFS копирование и так почти бесплатно (copy-on-write). | Symlink вместо копирования в dev-режиме. |

## 4. Архитектура и код

| # | Пр. | Оц. | Проблема | Предложение |
|---|-----|-----|----------|-------------|
| ~~4.1~~ ◐ | P1 | L | **Сделана главная часть.** Компоненты больше не write-once: рекордер, аудио-цепочка (VAD/шумоподавление/громкость), wake-движок и модель распознавания пересобираются на лету. Модуль `reconfigure` решает, что именно пересобрать при изменении настроек (чистая функция `plan`, покрыта тестами), IPC-действие `apply_settings` применяет их к работающему ассистенту — перезапуск больше не нужен. **Осталось:** сгруппировать компоненты в `struct Assistant` с явным `Config` вместо статиков — это чисто архитектурное, пользовательской разницы не даёт. | Собрать в `struct Assistant { recorder, stt, wake, vad, intent, … }` с явным `Config` → `build()`. Модули — обычные структуры с методами; глобальные `static` только для логгера. |
...
| ~~4.4~~ ✅ | P2 | S | **Сделано иначе, чем планировалось.** Enum невозможен: значение — либо id встроенного бэкенда, либо id модели из каталога, набор открыт по дизайну. Вместо этого добавлена валидация при записи по каталогу (`models::validate_backend`) — неизвестный id теперь отклоняется с перечислением допустимых, а не принимается с тихим откатом на дефолт. | `#[derive(Serialize, Deserialize)] enum` с `#[serde(rename_all = "kebab-case")]`, единый источник правды для GUI (отдавать список вариантов из бэкенда через tauri-команду). |
...
| ~~4.6~~ ✅ | P2 | M | TypeScript-типы событий и действий генерируются из Rust (`ts-rs`) в `frontend/src/lib/ipc-types.ts`, фронтенд их импортирует — обработчик событий теперь проверяется компилятором, нетипизированный `sendAction` убран. CI следит, что сгенерированный файл не отстал. Добавлена версия протокола: ассистент сообщает её в `started`, GUI и CLI предупреждают о несовпадении. | Генерировать TS-типы из Rust (`ts-rs`/`specta`) или хотя бы один общий JSON-schema-тест. |
| ~~4.7~~ ✅ | P2 | S | CLI переписан в тонкий IPC-клиент: `say`, `status`, `watch`, `stop`, `reload`, `apply-settings`, `mute`/`unmute` работают с запущенным ассистентом, плюс офлайн-команды `commands`, `phrases`, `settings`, `devices`, `models`. Зависимости от `intent-classifier`, `tokio` и моделей убраны — раньше CLI был вторым урезанным ассистентом и отвечал из собственного состояния, а не из того, с которым говорит пользователь. | Либо сделать CLI тонким клиентом IPC (`jarvis-cli say "открой браузер"`, `status`, `stop`), либо убрать. |
...
| ~~5.4~~ ◐ | P2 | S | **Частично.** `app.db` теперь сохраняется с правами 0600. Keychain отложен: оба ключа (`picovoice`, `openai`) сейчас не используются ни одним кодовым путём (Porcupine не поддерживается, ChatGPT не реализован) — переносить их в keychain имеет смысл вместе с появлением реального потребителя. | Keychain через `keyring` crate. |
