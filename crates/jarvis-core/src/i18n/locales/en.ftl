...
settings-ai = AI answers
settings-ai-desc = When no command matches, the question goes to Google Gemini.
settings-ai-key = Gemini key
settings-ai-key-desc = Get one at aistudio.google.com. Stored locally in the settings file.
settings-ai-model = Model
settings-ai-model-desc = Defaults to gemini-3.5-flash.
settings-ai-fallback = Answer questions
settings-ai-fallback-desc = Ask the model when no command was found.
settings-ai-speak = Speak answers
settings-ai-speak-desc = Read the answer out loud.
models-recommended = suggested
settings-vosk-accuracy-title = The small model struggles in noise
settings-tts-engine = Speech engine
settings-tts-engine-desc = Phrases generated in advance - nearly every command result, the time, the date and the battery among them - always use the assistant's voice. The engine speaks the rest.
settings-tts-system = System voice (instant)
settings-tts-clone = Jarvis voice (a few seconds)
settings-tts-clone-title = The Jarvis voice can be enabled
settings-tts-clone-hint = The voice cloning component is not installed. Run scripts/voice-clone/install.sh - it pulls in about 4 GB and is not part of the app. The assistant can then say any text in its own voice.
settings-vosk-accuracy-desc = Only a compact model is installed. In measurements with background noise the large model made about 2.5x fewer errors. The wake word keeps using a small model - large ones cannot do it - so do not delete it.
