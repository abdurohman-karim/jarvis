...
settings-ai-speak-desc = Читать ответ системным голосом.
models-recommended = советуем
settings-vosk-accuracy-title = В шуме модель ошибается чаще
settings-vosk-accuracy-desc = Сейчас установлена только компактная модель. На замерах с фоновым шумом большая русская модель давала в 2.5 раза меньше ошибок и лучше слышала слово «Джарвис». Её можно скачать ниже.
settings-tts-engine = Чем озвучивать
settings-tts-engine-desc = Заранее записанные фразы всегда звучат голосом ассистента; для остального выбирается движок.
